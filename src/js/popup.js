document.addEventListener('DOMContentLoaded', () => {
  ...
  function loadLanguageSettings() {
    if (window.ExtCompat) {
      window.ExtCompat.storageGet('language', (result) => {
        const language = result.language || window.ExtCompat.getUILanguage() || 'en';
  ...
  function getCurrentTabUrl() {
    try {
      window.ExtCompat.tabsQuery({ active: true, lastFocusedWindow: true }, (tabs) => {
        if (tabs && tabs.length > 0) {
          const tab = tabs[0];
          if (!tab.url || tab.status === "loading") {
            window.ExtCompat.tabsGet(tab.id, (updatedTab) => {
              if (updatedTab && updatedTab.url) {
                processTabUrl(updatedTab.url);
              } else {
                window.ExtCompat.executeScript(tab.id, {
  ...
  function loadSavedCustomParams() {
    window.ExtCompat.storageGet('customParams', (result) => {
  ...
  function exportHistory() {
    ...
    window.ExtCompat.storageGet('history', (result) => {
  ...
  function saveToHistory(link) {
    window.ExtCompat.storageGet('history', (result) => {
      ...
      window.ExtCompat.storageSet({ history: history }, () => {
  ...
  function clearHistory() {
    window.ExtCompat.storageSet({ history: [] }, () => {
      updateHistoryDisplay([]);
    });
  }

  function openSettings() {
    window.ExtCompat.openOptionsPage();
  }
});
